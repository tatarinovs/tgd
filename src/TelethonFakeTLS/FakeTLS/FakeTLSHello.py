...
class MTProxyFakeTLSClientCodec:
    ...
    def __init__(self, secret: str):
        raw = secret.lower()
        hex_str = secret if raw.startswith('ee') else f'ee{secret}'
        try:
            self.secret: bytes = bytes.fromhex(hex_str)
        except (ValueError, TypeError):
            self.secret: bytes = decode_b64(secret if secret.startswith('7') else f'7{secret}')

        self.domain: bytes = self.secret[17:]
        self.secret: bytes = self.secret[1:17]
        self.is_pkt_changed: bool = True
        self.pkt: bytes = b''
        # FIX CONCURRENCY: Create a deep copy of the class-level template for this instance!
        self.client_hello_dict = self.__class__.client_hello_dict.copy()

    # set or get parameters from client hello dict
    def client_hello(self, key: str, value: Union[bytes, int, str, None] = None,
                     ret_type: type = bytes) -> Union[bytes, int, str, None]:
        if value is None:
    ...
    def fix_padding(self):
        ...
        padding_len = max(0, 517 - len(self.glue_pkt()))
        self.client_hello('ext_padding_len', padding_len)
        if padding_len > 0:
            self.client_hello('ext_padding', Eh.myrandom.getrandbytes(padding_len))
